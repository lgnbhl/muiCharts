...
url_api_with_dropdown <- "https://mui.com/x/api/charts/animated-area/"

html_components <- url_api_with_dropdown |>
  read_html() |>
  html_elements(".css-uscrbs") # 'Component API' dropdown

components <- html_components |>
  html_text2()

components_href <- html_components |>
  html_children() |>
  html_attr("href") |>
  na.omit()

components_url_api <- paste0("https://mui.com", components_href)

df_components <- tibble(
  components = components,
  components_url_api = components_url_api
)
...
components_text <- purrr::map(
  .x = df_components$components,
  .f = create_component
) |>
  unlist()

components_text_header <- paste0(
  "## Script generated automatically from 'inst/helpers/documentation/generate-documentation.R' ##
...
base_url <- "https://mui.com/material-ui/api/"

get_component_props <- function(url) {
  url |>
    read_html() |>
    html_element(".css-1a2wx4v") |> # Props table node
    html_table() |>
    mutate(url = url)
}

components_props <- purrr::map_dfr(
  .x = components_url_api,
...
components_props_cleaned <- components_props |>
  ...
  mutate(
    Default = gsub("true", "TRUE", Default),
...
list <- purrr::map(
  .x = unique(components_props_cleaned$url),
  .f = create_component_items
) |>
  unlist()
...
init_test_component <- function(el) {
  ...
  test_script <- paste0(
    'test_that("',
    el,
    '() returns shiny.tag, correct name and value", {\n',
    '   expect_equal(class(',
    el,
    '()), "shiny.tag")\n',
    '   expect_equal(environment(',
    el,
    '()[["children"]][[2]])[["data"]][["name"]], "',
    el,
    '")\n',
    '   expect_equal(environment(',
    el,
    '("Test")[["children"]][[2]])[["data"]][["props"]][["value"]][["children"]], "Test")\n',
    '})\n'
  )
